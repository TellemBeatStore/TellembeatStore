...
function LatestBeats({ onNavigate }) {
   ...
   * LOAD PUBLIC BEATS
   *
   * Removed beats remain in Supabase for producer
   * appeals/history, but never appear publicly.
   */
  useEffect(() => {
    let cancelled = false;
  ...
  /*
   * OPEN A SHARED BEAT AUTOMATICALLY
   *
   * Always verify the beat directly in Supabase.
   *
   * This prevents:
   * - removed beats opening from old links
   * - stale browser state showing removed beats
   * - previously shared beat URLs bypassing removal
   */
  useEffect(() => {
  ...
  /*
   * LOAD COMMENTS FOR SELECTED BEAT
   */
  useEffect(() => {
    if (!selectedBeat?.id) {
      setComments([]);
  ...
  function getProducer(beat) {
    return beat?.producers || null;
  }

  /*
   * DOWNLOAD BEAT WITH REAL BEAT NAME
   */
   ...
   * OPEN BEAT
   *
   * Verify the beat is still active directly in
   * Supabase before displaying the detail page.
   */
   ...
   *
   * Extra safety filter:
   * Never allow a removed beat to appear in a
   * producer catalog even if state contains stale data.
   */
